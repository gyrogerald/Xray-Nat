...
touch /etc/nginx/sites-available/alone
cat <<EOF >>/etc/nginx/sites-availabe/alone
server {
	listen 81;
	listen [::]:81;
	server_name ${domain};
...
EOF
ln /etc/nginx/sites-available/alone /etc/nginx/sites-enabled/alone
...
cd
echo "Install XRAY Service Berhasil..."
sleep 3
# echo "Rebooting..."
# sleep 3
# reboot
